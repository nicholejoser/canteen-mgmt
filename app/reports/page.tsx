...
import { weeklySales, orders, menuItems, users } from '@/data/mockData';
import {
  DollarSign,
  TrendingUp,
  ShoppingCart,
...
export default function ReportsPage() {
  ...
  const categoryBreakdown = menuItems.reduce((acc, item) => {
    acc[item.category] = (acc[item.category] || 0) + item.soldToday * item.price;
    return acc;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header title="Reports & Analytics" subtitle="Comprehensive insights into your canteen operations" />
          ...
          <div className="card bg-gradient-to-br from-green-500 to-green-600 text-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold mt-1">${totalWeeklyRevenue.toLocaleString()}</p>
                <div className="flex items-center gap-1 mt-2">
                  <ArrowUpRight className="w-4 h-4" />
                  <span className="text-sm font-semibold">+12.5% vs last week</span>
                </div>
              </div>
          ...
          <div className="card bg-gradient-to-br from-blue-500 to-blue-600 text-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-1 mt-2">
                  <ArrowUpRight className="w-4 h-4" />
                  <span className="text-sm font-semibold">+8.3% vs last week</span>
                </div>
              </div>
          ...
          <div className="card bg-gradient-to-br from-purple-500 to-purple-600 text-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-1 mt-2">
                  <ArrowUpRight className="w-4 h-4" />
                  <span className="text-sm font-semibold">+3.7% vs last week</span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-1 mt-2">
                  ...
                  <span className="text-sm font-semibold">-1.2% vs last week</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              {Object.entries(categoryBreakdown)
                ...
                .map(([category, revenue]) => {
                  const totalCatRevenue = Object.values(categoryBreakdown).reduce((a, b) => a + b, 0);
                ...
                <div key={method} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      method === 'cash' ? 'bg-green-100' : method === 'card' ? 'bg-blue-100' : 'bg-purple-100'
                    }`}>
                      <DollarSign className={`w-5 h-5 ${
                        method === 'cash' ? 'text-green-600' : method === 'card' ? 'text-blue-600' : 'text-purple-600'
                      }`} />
                    </div>
                    <div>
                      <p className="font-semibold text-gray-900 capitalize">{method}</p>
                      <p className="text-xs text-gray-500">
                        {orders.filter(o => o.paymentMethod === method).length} transactions
                      </p>
                    </div>
                  </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="text-left py-3 px-4 text-sm font-semibold text-gray-600 rounded-l-lg">Day</th>
                    ...
                    <th className="text-right py-3 px-4 text-sm font-semibold text-gray-600 rounded-r-lg">Avg/Order</th>
                  </tr>
                </thead>
                <tbody>
                  {weeklySales.map((day) => (
                    <tr key={day.day} className="border-b border-gray-50">
                      ...
                      <td className="py-3 px-4 text-right font-semibold text-gray-900">${day.sales.toLocaleString()}</td>
                      <td className="py-3 px-4 text-right text-gray-600">{day.orders}</td>
                      <td className="py-3 px-4 text-right text-gray-600">${(day.sales / day.orders).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-gray-50 font-bold">
                    <td className="py-3 px-4 text-gray-900 rounded-bl-lg">Total</td>
                    <td className="py-3 px-4 text-right text-primary-600">${totalWeeklyRevenue.toLocaleString()}</td>
                    <td className="py-3 px-4 text-right text-primary-600">{totalWeeklyOrders}</td>
                    <td className="py-3 px-4 text-right text-primary-600 rounded-br-lg">${avgOrderValue.toFixed(2)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
